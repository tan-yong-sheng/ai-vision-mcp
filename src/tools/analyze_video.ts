...
export async function analyze_video(
  ...
  try {
    ...
    if (needsProcessing) {
        // Warning: Preprocessing (trim/resize) only supports local files.
        // URL sources are passed through without preprocessing.
        if (args.videoSource.startsWith('http') || args.videoSource.startsWith('gs://') || args.videoSource.startsWith('files/')) {
            console.warn('[analyze_video] Video preprocessing (trim/resize) is not supported for remote URLs. Skipping preprocessing.');
        } else {
