...
def get_available_images():
    """Get list of available test images in input directory."""
    input_dir = Path(__file__).parent / "input"
    # Look for images with corresponding ground truth JSON files
    gt_files = sorted(input_dir.glob("ground_truth_*.json"))
    image_ids = []
    for gt_file in gt_files:
        img_id = gt_file.stem.replace("ground_truth_", "")
        # Check if image exists (either .png or .jpg)
        if (input_dir / f"{img_id}.png").exists() or (input_dir / f"{img_id}.jpg").exists():
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Error: image_id or --batch-rerun is required")
        print("Usage: python orchestrator.py <image_id>")
        print("       python orchestrator.py --batch-rerun <image_ids>")
        print("       python orchestrator.py --list")
        print("Example: python orchestrator.py image_001")
        print("Example: python orchestrator.py --batch-rerun image_001,image_002,image_003")
        print("Example: python orchestrator.py --list")
        sys.exit(1)
    ...
    if sys.argv[1] == "--batch-rerun":
        if len(sys.argv) < 3:
            print("Error: image IDs required for batch rerun")
            print("Usage: python orchestrator.py --batch-rerun image_001,image_002,image_003")
            print("       python orchestrator.py --batch-rerun image_001-image_010")
            sys.exit(1)
        ...
        print(f"\n{'='*60}")
        print(f"ORCHESTRATOR: Dataset-based Segmentation Analysis")
        print(f"Image ID: {image_id}")
        print(f"{'='*60}")

        # Check if ground truth exists
        gt_path = Path(__file__).parent / "input" / f"ground_truth_{image_id}.json"
        if not gt_path.exists():
            print(f"❌ Error: ground_truth_{image_id}.json not found in input/")
            sys.exit(1)

        # Step 1: Run segmentation (no generation step for dataset-based workflow)
        segment_cmd = ["python", "scripts/test_segmentation.py", image_id]
        if not run_command(segment_cmd, "Run segmentation analysis"):
            sys.exit(1)

        # Step 2: Evaluate results
        eval_cmd = ["python", "evaluation/evaluate_script_accuracy.py", image_id]
        if not run_command(eval_cmd, "Evaluate accuracy metrics"):
            sys.exit(1)

        # Step 3: Display and log results
        results = load_evaluation_results(image_id)
        print_results_summary(image_id, results)
        append_to_result_md(image_id, results)

        print(f"✓ Analysis for {image_id} completed successfully")
